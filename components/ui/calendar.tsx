...
function Calendar({
  ...
  const availableDates = availableEvents.map((event) => new Date(event.date));
  const isEmpty = availableDates.length === 0;
  // Disable all days that are not in availableDates
  const isDateDisabled = (date: Date) => {
    if (isEmpty) {
      return true; // Disable all dates if no availableEvents
    }
  ...
  return (
    <DayPicker
      ...
      disabled={isDateDisabled} // Disable dates that are not in availableDates
